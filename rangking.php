		...
		?>
		            <tr>
						...
						<td>
							...
							$pro->insertHasil($row1['nama_alternatif'],$hasil['bbn']);
							?>
						</td>
		            </tr>
		<?php
...
?>
			<table width="100%" class="table table-striped table-bordered">
				<thead>
					<tr><td>Nama Kandidat</td><td>Rangking</td></tr>
				</thead>
				<tbody>
				<?php
				$hasilnya = $pro->readAllHasil();
				$rank = 1;
					while ($row2 = $hasilnya->fetch(PDO::FETCH_ASSOC)){
						?>
						<tr><td><?=$row2['alternatif']?></td><td><?=$rank++?></td></tr>
						<?php
					}
				?>
				</tbody>
